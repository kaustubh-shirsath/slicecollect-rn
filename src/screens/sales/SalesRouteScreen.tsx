...
export default function SalesRouteScreen({ navigation }: Props) {
  ...
  const load = (lat: number, lng: number, isReroute = false) => {
    ...
    if (isReroute) {
      setRerouting(true)
      setTimeout(() => {
        const stops = buildSalesRoute(username, lat, lng)
        setRouteStops(stops)
        setRerouting(false)
      }, 2200)
    } else {
  ...
  const timeline: Array<{ stop: SalesRouteStop; state: NodeState; index: number }> = [
    ...
    <View className="flex-1 bg-[#F0F4F7]">
          ...
          <View className="flex-row items-center justify-between">
            <View>
              <View className="flex-row items-center gap-2">
                ...
                <Text className="text-white font-medium text-lg">Sales Route</Text>
              </View>
              {!loading && (
      ...
      {rerouting && (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(9,11,12,0.80)', zIndex: 100, alignItems: 'center', justifyContent: 'center' }}>
          <Animated.View style={{ transform: [{ scale: pulseAnim }], alignItems: 'center', gap: 16 }}>
            <Animated.View style={{
              width: 72, height: 72, borderRadius: 36, backgroundColor: '#D30AD7',
      ...
      {loading ? (
        <View className="flex-1 items-center justify-center gap-3">
          ...
          <Text className="text-black/40 text-sm">Optimising your sales route…</Text>
        </View>
      ) : timeline.length === 0 ? (
        <View className="flex-1 items-center justify-center">
          <Text className="text-3xl mb-3">🎉</Text>
          <Text className="text-sm text-black/40">All merchants visited for today!</Text>
        </View>
      ) : (
        <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
          {timeline.map(({ stop, state, index }, ti) => {
            ...
            const bc = BUSINESS_TYPE_COLORS[stop.merchant.businessType] ?? { bg: '#F0F4F7', text: 'rgba(0,0,0,0.5)' }

            return (
              <View key={stop.merchant.merchantId + ti}>
                {/* Stop row */}
                <View className="flex-row items-center gap-3">
                  {/* Node */}
                  <View className="w-9 items-center">
                    <View className={`items-center justify-center rounded-full border-2 ${
                  ...
                  <TouchableOpacity
                    className="flex-1"
                    onPress={() => navigation.navigate('SalesMerchantDetail', { merchant: stop.merchant, fromScreen: 'SalesRoute' })}
                  >
                    <View
                      ...
                      style={
                        state === 'done' ? { opacity: 0.55 }
                        : state === 'current' ? { elevation: 4, shadowColor: '#D30AD7', shadowOpacity: 0.12, shadowRadius: 20, shadowOffset: { width: 0, height: 4 } }
                      ...
                      {state === 'current' && (
                        <View className="mt-3 flex-row items-center justify-between">
                          <View className="flex-row items-center gap-1 bg-[#D30AD7]/10 px-2.5 py-1 rounded-full">
                            ...
                            <Text className="text-[10px] font-medium text-[#D30AD7]">Next stop</Text>
                          </View>
                          <TouchableOpacity
                            onPress={() => navigation.navigate('SalesCollect', { merchant: stop.merchant })}
                            className="bg-[#D30AD7] px-4 py-1.5 rounded-full"
                          >
                            <Text className="text-xs text-white font-medium">Collect →</Text>
                          </TouchableOpacity>
                        </View>
                      )}
                    </View>
                ...
                {!isLast && (
                  <View className="flex-row" style={{ height: 32 }}>
                    <View className="w-9 items-center">
                      <View className="w-px flex-1" style={{ backgroundColor: state === 'done' ? 'rgba(0,166,62,0.35)' : 'rgba(0,0,0,0.15)' }} />
                    </View>
                    {distNext > 0 && (
          ...
          {pendingStops.length === 0 && totalVisited > 0 && (
            <View className="items-center py-8">
              ...
              <Text className="text-sm font-medium text-black/60">All merchants visited today!</Text>
            </View>
          )}
        </ScrollView>
      )}
